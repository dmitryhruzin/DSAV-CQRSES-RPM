#!/usr/bin/env python3
"""Export per-machine measured/predicted/MRE tables for Exp 6.

Builds for each of 4 machines:
  - Measured: from load logs (http.request.durationMs per class + RC_lag via event.handle)
  - Predicted: from JMT exp6 CSVs (per class, with RC_lag = Stvor/Onovl + RC convolution
...
def short_machine(m):
    return m.replace("-classical_cqrs", "")

for stat in ("mean", "median", "p95"):
    print(f"\n## {stat.upper()} per machine, Exp 6 (мс)\n")
...
print("\n## Aggregate |MRE| Exp 6 per machine (mean of 4 classes)\n")
