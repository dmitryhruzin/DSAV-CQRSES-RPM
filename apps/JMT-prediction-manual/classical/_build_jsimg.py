...
MACHINES = ["m7i-gp3-classical_cqrs", "m7i-io2-classical_cqrs", "m7a-gp3-classical_cqrs", "m7a-io2-classical_cqrs"]
...
ZAPYTY_CENTER_M7A_FACTOR = 0.75

# Per-class σ (tail) multipliers for exp12 classical: thin GET tail (lower p95),
# fatten Stvor/Onovl command tail (raise under-predicted p95). RC unchanged.
# m7a: applied in Exp-6 formula (mean-preserving via μ recompute).
# m7i: applied in _calibrate_m7i init so centers re-converge under new σ.
ZAPYTY_SIGMA_FACTOR = 0.85
CMD_SIGMA_FACTOR = 1.30


def sigma_class_factor(fit_cell: str) -> float:
    if "Zapyty" in fit_cell:
        return ZAPYTY_SIGMA_FACTOR
    if "Stvor" in fit_cell or "Onovl" in fit_cell:  # command cells (RC excluded)
        return CMD_SIGMA_FACTOR
    return 1.0
...
def get_calibrated_params(machine: str, fit_cell: str, method: str, experiment: str,
                          k_disk: float | None, base_machine: str = "m7a-gp3-classical_cqrs") -> dict:
    ...
    """
    is_predicted = (machine == "m7a-io2-classical_cqrs")
    ...
    if experiment in ("exp1", "exp4"):
        ...
        cell_seq = FITS["perMachine"][src_machine][method][fit_cell]
        cell_load = FITS_LOAD["perMachine"][src_machine][method][fit_cell]
        median_seq = cell_seq["median"] if cell_seq["median"] > 0 else cell_seq["mean"]
        sigma = cell_load["exp1"]["sigma"]
        ...
        if src_machine.startswith("m7i"):
            ...
            median_seq = cell_seq["median"] if cell_seq["median"] > 0 else cell_seq["mean"]
            sigma = cell_load["exp1"]["sigma"]
            if experiment in ("exp12", "exp13", "exp14", "exp16"):
                if method == "GET":
                    median_seq = median_seq * ZAPYTY_CENTER_M7A_FACTOR
                # per-class tail tuning (mean preserved by μ recompute below)
                sigma = sigma * sigma_class_factor(fit_cell)
            mu_ms = math.log(median_seq) - sigma * sigma / 2.0
...
def compute_k_disk(fits: dict | None = None, center: str = "mean"):
    ...
    if fits is None:
        fits = FITS
    gp3 = fits["perMachine"]["m7i-gp3-classical_cqrs"]
    io2 = fits["perMachine"]["m7i-io2-classical_cqrs"]
    K = {}
