#!/usr/bin/env node
// Same logic as _analyze_seq.mjs but processes *-load logs from logs10/.
// Writes log-analisis-load.md.
...
const OUT = join(HERE, 'log-analisis-load.md')
...
function renderTable(metrics, samples) {
...
function main() {
  ...
  const out = []
  out.push('# Метрики *-load логов (после 10s skip + retry-strip)')
  out.push('')
  ...
  out.push('')
  out.push('В отличие от сырых `logs10/`, здесь `event.handle.durationMs` нормализован: для каждого спана с признаком retry (`originalDurationMs > 950ms`) вычтено `N × 1000ms`, где `N` — количество ретраев VersionMismatch-логики ([_strip_retries.mjs](_strip_retries.mjs)).')
  out.push('')
  out.push('Формулы и набор метрик идентичны [log-analisis-seq.md](log-analisis-seq.md) — отличается только источник (логи под нагрузкой).')
  out.push('')
  ...
  for (const fname of LOG_FILES) {
    ...
    for (const m of ['GET', 'POST', 'PATCH']) {
      ...
      out.push('')
      out.push(renderTable(metrics[m], samples[m]))
      out.push('')
    }
  }
