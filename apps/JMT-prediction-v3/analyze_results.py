#!/usr/bin/env python3
"""Analyse JMT simulation outputs across all three distribution variants (v3).

V3: ground truth for MRE is the M7a.io2 Load distribution *after the first 10
seconds of the log are skipped* — matching ``apps/logs/analyz-10.md``. The
load distribution is recomputed live with the same 10s warm-up skip used by
``extract_demands.py``.
...
Writes ``analysis_v3.json`` and prints a comparison table.
"""
...
def main():
    ...
    # V3: all 9 stats including min/max/range/variance/stdev.
    ...
    (HERE / "analysis_v3.json").write_text(
